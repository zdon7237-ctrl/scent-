...
RESEND_API_KEY
EMAIL_FROM
BLOB_STORE_ID + VERCEL_OIDC_TOKEN（新版）或 BLOB_READ_WRITE_TOKEN（旧版）
UPSTASH_REDIS_KV_REST_API_URL（Marketplace）或 UPSTASH_REDIS_REST_URL（旧版）
UPSTASH_REDIS_KV_REST_API_TOKEN（Marketplace）或 UPSTASH_REDIS_REST_TOKEN（旧版）
```
